$.when(
    ...
    DrawMode.prototype = {
        ...
        getUserByRealId: function(user_RealId){
            var user = filterOne(this.LotteryBox, function(user){
                return user.real_id == user_RealId;
            });
            return user;
        },
        setCurrent: function() {
    ...
    var initStartUpEvent = function(){ //要啟動系統後才會監聽的事件
        ...
        $('.icon-gift').bind('click', function(){
            if($('.draw-start').length != 0){
                if(!confirm('抽獎過程尚未結束，確定要離開？')) {
                    ...
                    myFirebaseRef.ref('auth/gua_result/').remove();
        ...
        $('.toolbox').on('click', '.btn i', function(){
            ...
            if($this.is('.icon-user')){
                ...
                var csvRows = [['抽獎聯編號', '會員編號','姓名','職務','1級單位','獎項編號','獎項名稱','獎項內容'].join(',')+',,'+gift_chart_rows[0]], line;
                for(var i = 0, n = user_array.length; i < n; ++i){
                    var user = user_array[i];
                    line = [];
                ...
                this.$dom.find('.slots-bar-button').unbind('click').one('click', function(){
                    ...
                    $({}).queue(function(_) {
                        ...
                        if(!_dom.drawing_page.is('.active')) {
                            ...
                            user = that.getUser(); //抽出!!
                            var sn = '0'+user.sn; //前面補零，在 substr(-100,1) 時會取到 0，即可處理「會員編號不足六碼時要補零」的機制
                        }
        ...
        new DrawMode({
            ...
            $dom: $([
                '<div class="drawmode-basic">',
                    ...
                    '<div class="drawmode-group"></div>',
                    '<div class="drawmode-name"></div>',
                    '<input class="drawmode-inputRealId">',
            ...
            onDraw: function(deferred){
                ...
                updateView();
                that.$dom.find('.drawmode-inputRealId').bind('keydown', function(e){
                    if(e.keyCode == '13') {
                        that.$dom.find('.drawmode-stop').trigger('click');
                    }
                }).val('').focus();
                that.$dom.find('.drawmode-stop').unbind('click').one('click', function clickHandler(){
                    userRealId = that.$dom.find('.drawmode-inputRealId').val();
                    // user = that.getUserByRealId(userRealId); //輸入抽獎聯，請開啟此行
                    user = that.getUserBySn(userRealId); //輸入員工編號，請開啟此行
                    if(user) {
                        updateView();
                        deferred.resolve(user);
                    } else {
